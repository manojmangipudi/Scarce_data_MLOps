...
from scipy.stats import ks_2samp # Kolmogorov-Smirnov test for goodness of fit
import pandas as pd
import numpy as np
...
class DataValidation:
    def __init__(self, data_ingestion_artifact: DataIngestionArtifact, # input
                 data_validation_config: DataValidationConfig): # output
        try:
    ...
    def detect_dataset_drift(self, base_df: pd.DataFrame, current_df: pd.DataFrame, threshold=0.05) -> bool:
        try:
            ...
            for column in base_df.columns:
                ...
                is_sample_dist= ks_2samp(d1, d2) # Kolmogorov-Smirnov test for goodness of fit
                if threshold<=is_sample_dist.pvalue:
                    is_found=False
                else:
                    is_found=True
                    status=False
                report.update({column:{"pvalue":float(is_sample_dist.pvalue),
